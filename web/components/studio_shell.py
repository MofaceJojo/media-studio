...
def inject_studio_css() -> None:
    st.markdown(
        ...
        [data-testid="stSidebarCollapsedControl"] button {
          ...
          background: var(--studio-sidebar) !important;
...
def render_core_action(
    title: str,
    description: str,
    page: str,
    eyebrow: str,
) -> None:
    """Render one focused creation entry without dashboard-style clutter."""
    st.markdown(
        f"""
        <div class="studio-card">
          <div class="studio-context-label">{escape(eyebrow)}</div>
          <div class="studio-card-title">{escape(title)}</div>
          <div class="studio-card-copy">{escape(description)}</div>
        </div>
        """,
        unsafe_allow_html=True,
    )
    st.page_link(page, label=f"进入{title}", width="stretch")
