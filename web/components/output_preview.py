    ...
    with st.container(border=True):
        ...
        if st.button(tr("btn.generate"), type="primary", use_container_width=True):
            ...
            if tts_mode == "omnivoice":
                omni_cfg = config_manager.get_comfyui_config().get("tts", {}).get("omnivoice", {})
                omni_url = omni_cfg.get("base_url", "http://127.0.0.1:3900")
                omni_ok, omni_msg = check_omnivoice_health(omni_url)
                if not omni_ok:
                    st.error(omni_msg)
                    st.stop()

            if workflow_key and str(workflow_key).startswith("selfhost/"):
            ...
            try:
                ...
                if tts_mode in ("local", "omnivoice"):
                    gen_params["tts_voice"] = selected_voice
                    gen_params["tts_speed"] = tts_speed
                if tts_mode == "omnivoice" and tts_instruct:
                    gen_params["tts_instruct"] = tts_instruct
                if tts_mode == "comfyui":
...
def render_batch_output(morpheus_video_studio, video_params):
    ...
    with st.container(border=True):
        ...
        if not topics or not any(str(topic).strip() for topic in topics):
        ...
        if st.button(
            ...
            if shared_config["tts_inference_mode"] == "omnivoice":
                tts_instruct = video_params.get("tts_instruct")
                if tts_instruct:
                    shared_config["tts_instruct"] = tts_instruct
            else:  # comfyui
