...
class VideoAnalysisService(ComfyBaseService):
    ...
    - selfhost/analyse_video.json (default, cloud-based)
    - selfhost/analyse_video.json (local ComfyUI, future)
    
    Usage:
        # Use default (selfhost cloud)
        description = await morpheus_video_studio.video_analysis("path/to/video.mp4")
        
        # Use local ComfyUI (future)
        description = await morpheus_video_studio.video_analysis(
            "path/to/video.mp4",
            source="selfhost"
        )
        
        # List available workflows
        workflows = morpheus_video_studio.video_analysis.list_workflows()
    """
    ...
    async def __call__(
        ...
        source: Literal['selfhost'] = 'selfhost',
        ...
        **params
        ...
        Args:
            ...
            source: Workflow source - 'selfhost' (cloud, default) or 'selfhost' (local ComfyUI)
        ...
        Examples:
            # Simplest: use default (selfhost cloud)
            description = await morpheus_video_studio.video_analysis("temp/01_segment.mp4")
            ...
            description = await morpheus_video_studio.video_analysis(
                "temp/01_segment.mp4",
                workflow="selfhost/custom_video_analysis.json"
            )
        """
        from morpheus_video_studio.utils.workflow_util import resolve_workflow_path
        ...
        try:
            ...
            # Format 3: Selfhost raw_data (text file URL)
