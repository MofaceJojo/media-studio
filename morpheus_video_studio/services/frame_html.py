...
class HTMLFrameGenerator:
    ...
    async def generate_frame(
        ...
        frame_css = (ext or {}).get("_frame_css", "")
        context = {
            "title": title,
            "text": f'<span id="mpt-subtitle">{text}</span>' if frame_css and text else text,
            "image": image,
        }
        
        if ext:
            context.update({key: value for key, value in ext.items() if key != "_frame_css"})
        
        html = self._replace_parameters(self.template, context)
        if frame_css:
            html = html.replace("</head>", f"<style>{frame_css}</style></head>", 1)
            html = html.replace(
                "</body>",
                """
                <script>
                    const subtitle = document.getElementById("mpt-subtitle");
                    if (subtitle) document.body.appendChild(subtitle);
                </script>
                </body>
                """,
                1,
