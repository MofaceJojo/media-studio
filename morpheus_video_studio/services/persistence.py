...
from morpheus_video_studio.models.storyboard import Storyboard, StoryboardFrame, StoryboardConfig, ContentMetadata
...
class PersistenceService:
    ...
    def _config_to_dict(self, config: StoryboardConfig) -> Dict[str, Any]:
        ...
        return {
            ...
            "subtitle_customization_enabled": config.subtitle_customization_enabled,
            ...
            "subtitle_stroke_color": config.subtitle_stroke_color,
    ...
    def _dict_to_config(self, data: Dict[str, Any]) -> StoryboardConfig:
        ...
        return StoryboardConfig(
            ...
            stock_selection_mode=data.get("stock_selection_mode", "sequential"),
            subtitle_customization_enabled=data.get("subtitle_customization_enabled", False),
            ...
            subtitle_size=data.get("subtitle_size", 60),
            subtitle_stroke_color=data.get("subtitle_stroke_color", "#000000"),
            subtitle_stroke_width=data.get("subtitle_stroke_width", 1.5),
        )
    ...
    def _dict_to_frame(self, data: Dict[str, Any]) -> StoryboardFrame:
        ...
        if not math.isfinite(audio_duration) or audio_duration <= 0:
            audio_duration = self._probe_audio_duration(data.get("audio_path"))
            if audio_duration <= 0:
                audio_duration = float(data.get("duration", 0.0) or 0.0)
    ...
    def _probe_audio_duration(self, audio_path: Optional[str]) -> float:
        """Recover a missing persisted audio duration from the original file."""
        if not audio_path:
            return 0.0
        try:
            ...
            return max(float(output.strip()), 0.0)
        except (OSError, subprocess.SubprocessError, ValueError) as exc:
            logger.debug(f"Could not recover audio duration for {audio_path}: {exc}")
            return 0.0
