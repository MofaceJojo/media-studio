...
class FrameProcessor:
    ...
    async def __call__(
        ...
        if os.path.exists(existing_segment):
            ...
            if existing_duration > 0:
                ...
                if frame.audio_path:
                    frame.audio_duration = await self._get_audio_duration(frame.audio_path)
    ...
    async def _step_generate_audio(
        ...
        # Get audio duration
        frame.audio_duration = await self._get_audio_duration(audio_path)
        frame.duration = frame.audio_duration
