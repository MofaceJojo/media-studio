...
"""
Morpheus Video Studio Core - Service Layer
...
from morpheus_video_studio.services.image_analysis import ImageAnalysisService
...
from morpheus_video_studio.pipelines.custom import CustomPipeline
from morpheus_video_studio.pipelines.asset_based import AssetBasedPipeline


class MorpheusVideoStudioCore:
    """
    Morpheus Video Studio Core - Service Layer
    ...
    Usage:
        ...
        print(f"Using LLM: {morpheus_video_studio.llm.active}")
        print(f"Available TTS: {morpheus_video_studio.tts.available}")
    
    Architecture (Simplified):
        MorpheusVideoStudioCore (this class)
    ...
    def __init__(self, config_path: str = "config.yaml"):
        """
        Initialize Morpheus Video Studio Core
    ...
    async def initialize(self):
        ...
        if self._initialized:
            logger.warning("Morpheus Video Studio already initialized")
            return
        
        logger.info("🚀 Initializing Morpheus Video Studio...")
        ...
        logger.info("✅ Morpheus Video Studio initialized successfully\n")
    ...
    async def cleanup(self):
        ...
        Example:
            await morpheus_video_studio.cleanup()
        """
        if self._comfykit:
            ...
            try:
                ...
                logger.warning(f"ComfyKit cleanup skipped: {e}")
            except Exception as e:
    ...
    def project_name(self) -> str:
        ...
        return self.config.get("project_name", "Morpheus Video Studio")
    
    def __repr__(self) -> str:
        """String representation"""
        status = "initialized" if self._initialized else "not initialized"
        pipelines = f"pipelines={list(self.pipelines.keys())}" if self._initialized else ""
        return f"<MorpheusVideoStudioCore project={self.project_name!r} status={status} {pipelines}>"
