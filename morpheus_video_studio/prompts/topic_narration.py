...
_STRING_NARRATION_OUTPUT_EXAMPLE = """```json
{
  "narrations": [
...
_SCREEN_RANKING_OUTPUT_EXAMPLE = """```json
{
  "narrations": [
    {
      "narration": "第1项：《具体名称》｜自然串联下列证据的完整旁白",
      "item_name": "具体且可识别的影视条目名称",
      "character": "旁白中出现的可识别人物",
      "situation": "外部可见的剧情处境、地点或约束",
      "action_or_conflict": "镜头可以直接呈现、带对象或结果的行动或冲突",
      "selection_reason": "由上述具体事件直接支撑的入选理由"
    }
  ]
}
```"""
...
- Length limit: For Chinese, each narration must contain {min_words}～{max_words} Chinese characters (中文每段 {min_words}～{max_words} 个汉字); for space-separated languages, use {min_words}～{max_words} words
...
- Length check: For Chinese, self-check that each narration is不得少于 {min_words} 个汉字; if insufficient, add a concrete action, detail, example, or explanation instead of filler
...
3. Narrations must follow the language-aware length limit above; Chinese is counted by characters, not segmented words
...
def build_topic_narration_prompt(
    ...
    max_words: int,
    recipe_block: str = "",
    structured_screen_ranking: bool = False,
) -> str:
    """
    ...
    Args:
        ...
        structured_screen_ranking: Require evidence fields for screen rankings

    Returns:
        Formatted prompt
    """
    section = f"\n{recipe_block.strip()}\n" if recipe_block and recipe_block.strip() else ""
    if structured_screen_ranking:
        output_example = _SCREEN_RANKING_OUTPUT_EXAMPLE
        output_requirement = (
            "影视盘点的 narrations 数组必须由对象组成，每项完整包含 narration、"
            "item_name、character、situation、action_or_conflict、selection_reason；"
            "item_name 必须对应旁白标题，character 中的每个人物以及其余三个证据字段"
            "必须出现在 narration 对应内容中"
        )
    else:
        output_example = _STRING_NARRATION_OUTPUT_EXAMPLE
        output_requirement = '{"narrations": [narration array]} JSON object'
