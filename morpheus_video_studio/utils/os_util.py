...
Provides utilities for managing paths and files in Morpheus Video Studio.
"""
...
def get_morpheus_video_studio_root_path() -> str:
    """
    Get Morpheus Video Studio root path
    
    Uses MORPHEUS_VIDEO_STUDIO_ROOT environment variable to determine project root.
    ...
    env_root = os.environ.get("MORPHEUS_VIDEO_STUDIO_ROOT")
...
def ensure_local_ffmpeg_on_path() -> Optional[str]:
    ...
    """
    candidates = [
        Path(get_morpheus_video_studio_root_path()) / "tools" / "ffmpeg" / "bin",
...
def get_root_path(*paths: str) -> str:
    """
    Get path relative to Morpheus Video Studio root path
    ...
    """
    root_path = ensure_morpheus_video_studio_root_path()
    if paths:
...
def get_temp_path(*paths: str) -> str:
    """
    Get path relative to Morpheus Video Studio temp folder
...
def get_data_path(*paths: str) -> str:
    """
    Get path relative to Morpheus Video Studio data folder
