...
from morpheus_video_studio.utils.audio_timing import detect_audio_timing
...
class StandardPipeline(LinearVideoPipeline):
    ...
    def _validated_scene_drafts(params: dict) -> list[SceneDraft]:
        ...
        if scenes and any(
            scene.index != index for index, scene in enumerate(scenes, 1)
        ):
            raise ValueError("分镜草稿编号不连续")
        for scene in scenes:
            if not scene.narration.strip():
                raise ValueError(f"分镜 {scene.index} 缺少旁白")
        return scenes
    ...
    async def generate_content(self, ctx: PipelineContext):
        ...
        scenes = self._validated_scene_drafts(ctx.params)
        if scenes:
            if len(scenes) != n_scenes:
                raise ValueError("分镜草稿数量不匹配")
            ctx.narrations = [scene.narration for scene in scenes]
            logger.info(f"✅ Reused {len(ctx.narrations)} edited scene narrations")
            return
    ...
    async def plan_visuals(self, ctx: PipelineContext):
        ...
        scenes = self._validated_scene_drafts(ctx.params)
        if template_requires_media and scenes:
            if len(scenes) != len(ctx.narrations):
                raise ValueError("分镜草稿数量不匹配")

            if template_type == "video":
                prompt_type = "视频提示词"
                base_prompts = [scene.video_prompt for scene in scenes]
            else:
                prompt_type = "图片提示词"
                base_prompts = [scene.image_prompt for scene in scenes]

            for scene, base_prompt in zip(scenes, base_prompts):
                if not base_prompt.strip():
                    raise ValueError(f"分镜 {scene.index} 缺少{prompt_type}")

            prompt_prefix = ctx.params.get("prompt_prefix")
            media_config = self.core.config.get("comfyui", {}).get(template_type, {})
            ...
            logger.info(f"✅ Reused {len(ctx.image_prompts)} edited {prompt_type}")
            return
    ...
    async def post_production(self, ctx: PipelineContext):
        ...
        transition_audio_delay = max(
            0.0, float(ctx.params.get("transition_audio_delay", 0.0) or 0.0)
        )
        uses_sequential_audio = video_service._should_use_xfade(
            transition, transition_choices
        )
        ...
        delivery_path = user_specified_output or final_video_path

        if storyboard.config.subtitle_enabled:
            timing_semaphore = asyncio.Semaphore(4)

            async def detect_timing(audio_path: str):
                async with timing_semaphore:
                    return await asyncio.to_thread(detect_audio_timing, audio_path)

            audio_timings = await asyncio.gather(
                *(detect_timing(frame.audio_path) for frame in storyboard.frames)
            )
            timeline_segments = []
            final_frame_index = len(storyboard.frames) - 1
            for index, (frame, timing) in enumerate(
                zip(storyboard.frames, audio_timings)
            ):
                if uses_sequential_audio:
                    raw_audio_duration = timing.duration or frame.audio_duration
                    timeline_duration = raw_audio_duration + (
                        transition_audio_delay if index < final_frame_index else 0.0
                    )
                else:
                    timeline_duration = frame.duration
            ...
            cues = build_global_cues(timeline_segments)
            srt_path = str(Path(delivery_path).with_suffix(".srt"))
            Path(srt_path).parent.mkdir(parents=True, exist_ok=True)
            export_srt_cues(cues, srt_path)
            video_service.burn_subtitles(
                final_video_path,
                srt_path,
                final_video_path,
                font=storyboard.config.subtitle_font,
                position=storyboard.config.subtitle_position,
                color=storyboard.config.subtitle_color,
                size=storyboard.config.subtitle_size,
                stroke_color=storyboard.config.subtitle_stroke_color,
                stroke_width=storyboard.config.subtitle_stroke_width,
                cues=cues,
            )
            logger.info(f"📝 Subtitles exported and burned from: {srt_path}")
