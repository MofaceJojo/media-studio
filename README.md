...
- ✅ **栏目配方** —— 科普 / 中药 / 排名 / 医学四套垂类骨架，锁住文案结构，杜绝"写成人生感悟"
- ✅ **画面禁人物规则** —— 知识类栏目自动回避人物（本地模型画人会变形），转向药材、器物、古籍、药田等静物题材
- ✅ **古籍竖排模板** —— 引用《本草纲目》原文时用真实汉字排版渲染，不让 AI 画出鬼画符
- ✅ **BGM 自动闪避** —— 旁白响起时音乐自动压低，说完回升，音频听感专业化
- ✅ **配音自动降级** —— OmniVoice 卡死时自动切 Edge TTS 把这段配完，不中断整片生成
- ✅ **断点续传** —— 生成中断后重跑，已完成的分镜直接复用，只补缺口
- ✅ **视频风格化** —— 上传素材做滤镜换风格（秒级）或 AI 重绘成动画风（支持参考图引导画风）
...
- ✅ **2026-07 下旬**: 栏目配方新增画面规则（知识类禁人物）与证据分级；古籍竖排文字模板；BGM 自动闪避；OmniVoice 故障自动降级 Edge；断点续传；播客式数字人口播；字幕换行符泄漏修复；标题语言护栏
- ✅ **2026-07 中旬**: 视频风格化工坊（滤镜档 + AI 重绘档 + 参考图引导）；成片末段黑屏修复；自定义素材支持 OmniVoice 音色；移除 Seedance 付费入口
- ✅ **2026-07 上旬**: 镜头节奏硬约束与 YouTube 舒缓档；成片自检门；SRT 字幕导出；flux-schnell (GGUF) 工作流；修复 OmniVoice 闲置卡死；`video.py` 模块化重构；GitHub Actions CI
- ✅ **2026-07 上旬**: 转场不再吞旁白（垫尾帧 + 静音填充）；TTS 瞬时故障自动重试
...
| 文档 | 内容 |
|---|---|
| [docs/INSTALL.md](docs/INSTALL.md) | 完整安装教程（主程序 / LLM / ComfyUI 与模型清单 / OmniVoice 四层） |
| [docs/upgrade-roadmap-2026-07.md](docs/upgrade-roadmap-2026-07.md) | 升级路线图与同类项目对照结论 |
| [docs/HERMES-WORKFLOW.md](docs/HERMES-WORKFLOW.md) | Hermes 委派工作流（规格书七段式 + 五步验收清单） |
| [docs/FAQ_CN.md](docs/FAQ_CN.md) | 常见问题 |
