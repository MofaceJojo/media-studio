...
class TTSOmniVoiceConfig(BaseModel):
    """OmniVoice local API configuration"""
    base_url: str = Field(default="http://127.0.0.1:3900", description="OmniVoice local API base URL")
    model: str = Field(default="omnivoice", description="OmniVoice TTS model id")
    voice: str = Field(default="default", description="OmniVoice voice/profile id")
    speed: float = Field(default=1.0, ge=0.25, le=4.0, description="Speech speed multiplier")
    language: Optional[str] = Field(default=None, description="Language code or name")
...
class TTSSubConfig(BaseModel):
    ...
    inference_mode: str = Field(default="local", description="TTS inference mode: 'local', 'omnivoice', or 'comfyui'")
    local: TTSLocalConfig = Field(default_factory=TTSLocalConfig, description="Local TTS (Edge TTS) configuration")
    omnivoice: TTSOmniVoiceConfig = Field(default_factory=TTSOmniVoiceConfig, description="OmniVoice local API configuration")
...
class StockMaterialsConfig(BaseModel):
    """Stock material source configuration"""
    default_provider: str = Field(default="all", description="Default stock material provider: all, pexels, or pixabay")
