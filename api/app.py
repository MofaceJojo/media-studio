...
"""
Morpheus Video Studio FastAPI Application
...
from api.dependencies import shutdown_morpheus_video_studio
...
async def lifespan(app: FastAPI):
    ...
    """
    # Startup
    logger.info("🚀 Starting Morpheus Video Studio API...")
    await task_manager.start()
    logger.info("✅ Morpheus Video Studio API started successfully\n")
    
    yield
    
    # Shutdown
    logger.info("🛑 Shutting down Morpheus Video Studio API...")
    await task_manager.stop()
    await shutdown_morpheus_video_studio()
    logger.info("✅ Morpheus Video Studio API shutdown complete")


# Create FastAPI app
app = FastAPI(
    title="Morpheus Video Studio API",
    description="""
    ## Morpheus Video Studio - AI Video Generation Platform API
...
async def root():
    ...
    return {
        "service": "Morpheus Video Studio API",
        "version": "0.1.0",
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Start Morpheus Video Studio API Server")
    ...
    print(f"""
╔══════════════════════════════════════════════════════════════╗
║                    Morpheus Video Studio API Server                      ║
╚══════════════════════════════════════════════════════════════╝
