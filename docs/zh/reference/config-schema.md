...
comfyui:
  ...
  selfhost_api_key: ""
  selfhost_concurrent_limit: 1  # 并发限制 (1-10)
  selfhost_instance_type: ""  # 实例类型（可选，设为 "plus" 使用 48GB 显存）
...
- `selfhost_api_key`: Local ComfyUI API 密钥（使用云端工作流时必填）
- `selfhost_concurrent_limit`: 并发执行限制（1-10，普通会员默认为 1）
- `selfhost_instance_type`: 实例类型（可选）
  - 留空或不设置：使用 24GB 显存机器
...
- `default_workflow`: 默认视频生成工作流
  - `selfhost/video_wan2.1_fusionx.json`: 云端工作流（推荐，无需本地环境）
