...
comfyui:
  ...
  selfhost_api_key: ""
  selfhost_concurrent_limit: 1  # Concurrent limit (1-10)
  selfhost_instance_type: ""  # Instance type (optional, set to "plus" for 48GB VRAM)
...
### Local ComfyUI Cloud Configuration

- `selfhost_api_key`: Local ComfyUI API key (required for cloud workflows)
- `selfhost_concurrent_limit`: Concurrent execution limit (1-10, default 1 for regular members)
- `selfhost_instance_type`: Instance type (optional)
...
- `default_workflow`: Default video generation workflow
  - `selfhost/video_wan2.1_fusionx.json`: Cloud workflow (recommended, no local setup required)
