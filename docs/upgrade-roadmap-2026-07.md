...
## 5. 视频风格化功能（2026-07-08 方案定版）

用户需求：上传一个/一组视频（最长 3 分钟），转成其他风格。**明确取消过夜方案，全走快速路线。**

两档能力，放在资产库"风格化"入口，产物自动进素材库供视频工坊复用：

**A 档：风格滤镜（秒级，任意长度）**
- ffmpeg 滤镜链 + LUT：宫崎骏暖调 / 胶片 / 水墨淡彩 / 赛博霓虹 / 复古 VHS / 黑白电影
- 流程：上传 → 选风格 → 3 秒预览小样 → 批量导出

**B 档：AI 重绘（快速化三件套，上限 60 秒/段）**
- 提速手段：降帧到 8-12fps（动画本来就是这个帧率）+ LCM-LoRA（20 步→4-6 步，提速 3-4 倍）+ 512p 处理
- 结构锁定：ControlNet（canny 版走 ComfyUI 内置节点零插件；lineart 版需 controlnet_aux 插件，待用户授权安装）
- 预期速度：30 秒素材约 10 分钟出片；60 秒约 20 分钟
- 超过 60 秒的视频：引导走 A 档滤镜，或按镜头切段选重点段落重绘
- 已下载资产：control_v11p_sd15_lineart_fp16 / control_v11p_sd15_canny_fp16 / lcm-lora-sdv15

**版权提醒**：重绘不改变原素材版权归属，公开发布只用自有/无版权素材。
